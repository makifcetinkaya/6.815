...
def applyDoubleHomography(src1, src2, ws1, ws2, out, T, H2, bilinear=False):
    if bilinear:
        for y, x, in imIter(out):
            ...
            if isInside(P1, src1):
                pval1 = linearReconstructP(src1, P1)
                if isInside(P2, src2):
                    pval2 = linearReconstructP(src2, P2)
                    w1 = linearReconstructP(ws1, P1)
                    w2 = linearReconstructP(ws2, P2)
                    out[y,x] = (pval1*w1+pval2*w2)/(w1+w2)
                else:
                    out[y,x] = linearReconstructP(src1, P1)
                    
            elif isInside(P2,src2):
                out[y,x] = linearReconstructP(src2, P2)
    else:
        for y, x, in imIter(out):
            P1 = applyH2d(T, y, x)
            P2 = applyH2d(H2, y, x) 
            ...
            if isInside(P1, src1):
                pval1 = src1[P1[0], P1[1]]
                if isInside(P2, src2):
                    pval2 = src2[P2[0], P2[1]]
                    w1 = ws1[P1[0], P1[1]]
                    w2 = ws2[P2[0], P2[1]]
                    if w1+w2 > 0:
                        out[y,x] = (pval1*w1+pval2*w2)/(w1+w2)
...
def calcLinBlendWeights(im):
    h, w = im.shape[0:2]
    weights = np.zeros([h,w])    
    for j in range(h):
        for i in range(w):
            x_val = 1-2.0/w*math.fabs(i-w/2)
            y_val = 1-2.0/h*math.fabs(j-h/2)
            weights[j][i] = x_val*y_val
